...
// A run is `(source, ordinal)`: that pair is its identity in the archive and therefore the
// key here, because two sources number their runs from one each.
const keyOf = (run: RunView): string =>
  `${run.source.kind === 'live' ? 'live' : run.source.name}#${run.ordinal}`
</script>
