...
export const queue = (): Promise<QueueView> => invoke(Command.Queue)
...
export const queueAdd = (achievement: number): Promise<QueueView> =>
  invoke(Command.QueueAdd, { achievement })

export const queueRemove = (achievement: number): Promise<QueueView> =>
  invoke(Command.QueueRemove, { achievement })
...
export const queueMove = (
  achievement: number,
  to: number,
): Promise<QueueView> => invoke(Command.QueueMove, { achievement, to })
...
export const queueImportGoals = (): Promise<QueueView> =>
  invoke(Command.QueueImportGoals)
