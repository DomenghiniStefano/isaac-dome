...
// value, in both directions, and the attribute only repeated it — while ts-rs, whose
// serde attribute parser knows four keys and warns on every other one, printed
// `failed to parse serde attribute` on each build. The shape is pinned by
// `a_goal_id_is_a_bare_json_string_in_both_directions`, not by the attribute. The note
// is deliberately not a doc comment: ts-rs copies those into `types.ts`, and this is
// about our build, not about the contract.
