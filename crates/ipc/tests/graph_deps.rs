...
fn a(n: u32) -> graph::AchievementId {
    graph::AchievementId(n)
}

#[test]
fn a_row_requires_every_step_of_its_chain_and_nothing_else() {
    let deps = GraphDeps::from_chains([(a(10), vec![a(7), a(8)])]);
    assert!(deps.requires(a(10), a(7)));
    assert!(deps.requires(a(10), a(8)));
    assert!(!deps.requires(a(10), a(9)));
}
...
fn a_row_the_graph_cannot_compute_carries_no_constraint() {
    ...
    let deps = GraphDeps::from_chains([(a(10), Vec::new()), (a(7), Vec::new())]);
    assert!(!deps.requires(a(10), a(7)));
    assert!(!deps.requires(a(7), a(10)));
}
...
fn a_row_that_was_never_asked_about_is_not_a_constraint() {
    ...
    let deps = GraphDeps::from_chains([(a(10), vec![a(7)])]);
    assert!(!deps.requires(a(99), a(7)));
}
