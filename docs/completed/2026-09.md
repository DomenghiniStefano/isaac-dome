...
### 2026-09-18 — two screens looked at in a window, and both were wrong

Not a sub-project: the owner opened the app and reported two things, and the session went looking
for what each one actually was before writing it down. Both are entries now — **B64** (the Floor
screen) and **B65** (`Ctrl+Enter` in the palette) — and cards on the board.

**The Floor screen.** *You cannot tell where the rooms are, and the legend needs improving.* Read
against the code it is five defects with one cause. `fillOf` in `FloorGrid.vue` has a single token
for a painted cell, so the fourteen kinds the brush can paint come out as one grey square; the start
room carries `aria-current` and no paint; `FloorLegend.vue` is the brush picker and not a legend, so
`--color-floor-candidate-first/second/third` are never given words; only the Secret Room's
candidates reach the grid while three targets are listed underneath it; and a candidate row names
its cell by an index from 0 to 168, which is not a position anybody can find on a 13x13 drawing. The
cause under four of them is the same: a cell says one thing and the screen needs it to say two —
what you drew, and what the rules make of it.

**The palette's `Ctrl+Enter` is swallowed by reka-ui**, and that half is certain rather than
inferred. `ListboxRoot.onKeydownEnter` returns on `ctrlKey || metaKey || altKey` *before* it clicks
the highlighted row, so no `select` is emitted and `SearchPalette.openAt` is never reached. Our own
`useGestureModifiers` reads the modifier correctly and nothing ever asks it. `Ctrl`+click works,
because that path is the one the library filters out only for the keyboard. The footer draws the
gesture, which makes it a documented key that does nothing.

**The second half of that report was the more interesting one.** *I do not know whether Enter works*
has no answer in the code, only a prediction: the filter highlights the first item on every
keystroke, the rows come back 120 ms later, so for a query the backend answers the highlight is
placed on rows that have not mounted yet — or on the previous answer's, which unmount, and
`onKeydownEnter` skips an element that is no longer connected. Screens rows are computed
synchronously from the typed text and are the exception. The prediction written into B65 is
therefore: `Enter` works when the first row is a screen and does nothing when the row came from the
backend. A window settles it in a minute, and now it has something to disagree with.

**What the pair says about the process.** Neither screen had a group in *"What only a window can
say"*, because neither sub-project ever wrote one — the Floor report has no `- [ ]` in it at all.
That list is only as honest as what enters it: a screen that gathers no checks is not visible there
as outstanding, it is not visible there at all. Both new entries close with a line that puts their
group in it before the branch closes.

### 2026-09-17 (fourth) — the app opens by asking which save, and B14 comes along

Sub-project 3.8, `feature/welcome-flow`: B17's item 2 and B14 in one branch. Spec, plan and report
are `docs/superpowers/{specs,plans,reports}/2026-09-17-welcome-flow-*.md`; what is worth keeping
here is how the shape was decided and what the work found that nobody had asked about.

**Four decisions in conversation, and the second is the one with teeth.** A full-screen takeover;
**a state above the router and never a route**, because here a route *is* a tab and 3.7b's session
document would have saved and restored it — a state inside a document that describes contents. It
takes "you have to choose" and "nothing was found" alike, a card carries three counts plus a line
for what could not be read, and the same takeover is the way back from the indicator.

**What made the takeover safe was one line in `App.vue`**: `useWindowSession()` is called in the
setup, not in the template. So replacing the template's body unmounts neither the session
machinery nor the tabs store, and a window goes on holding its tabs behind the welcome. Had it
been mounted inside the markup, the whole design would have had to go the other way.

**Two entries were wrong about themselves, and both times the fix was to open the file they
named.** B17 promised a "KPI strip already drawn by Progress" that does not exist; B14 did not
know half its own backend was already in `discovery`. Same shape as B22's item 3 that morning.

**Three findings from writing it.** `ipc::Settings` crosses the IPC, so the two chosen folders
could not go in it — a `PathBuf` there puts the Windows username on the wire — and
`settings_file::save` now preserves them, once, instead of sixteen callers knowing. `select_profile`
searched with `Options::default()`, so a save in a hand-picked folder would have been refused as
unknown the moment you chose it. And the achievements denominator is `declared - 1`, because slot 0
is no achievement: this card is the first place a **user** ever sees that total, so the number had
to be decided rather than inherited.

**The gate lost three states, not two, and the tests moved instead of dying.** With the welcome
owning `needsChoice`, `none` **and** `failed`, `gateState` was left with "the profile is there" and
"we do not know yet". Its three retired tests are asserted in `welcomeView.test.ts` now — a
deletion would have been silent, and this repo has a floor precisely because of that.

**The documentation could not settle one thing, and it is written down rather than guessed.**
`blocking_pick_folder` must not run on the main thread; which thread a synchronous Tauri command
runs on is not documented. So the two commands are `async` with the callback form and a
`tauri::async_runtime::channel`, verified in the installed crate's source. That choice is exactly
the check the window still owes.

- [x] 3.8 built, `pnpm check` green, B17 and B14 closed.
- [ ] Nobody has opened a window on any of it. Six lines in *"What only a window can say"*, four of
      which `pnpm ui:dev` can answer on its own.

### 2026-09-17 (third) — B54's page-own headings, read one by one, and a premise that inverts

B54's third family: 42 titles seen once, of which 9 already carried a written refusal, 1 was the
anchor already closed, and 10 are the second-subject family that waits on a decision. **22 were
left, and the entry asked only that they be read and recorded.**

**The record is a test, and that was the whole choice.** Prose in a document does not fail when
somebody adds one of these spellings by reflex; `a_page_own_heading_is_read_and_stays_out` does.
Shown rather than assumed: adding `"strategy and items"` to the `Strategies` arm turns it red with
the page named. Twenty-two assertions of `None` would otherwise be a test that cannot fail, which
this repo already knows is worse than no test.

**What reading them actually gave**, beyond the tick. Two of the 22 are not content at all:
`[[Monsters]]` on boss/Great Gideon is a heading that is a **link**, and its section has no body
— the 54 lines the counter attributes to it are two `=== … Waves ===` tables *under* it — while
`Combinations` on collectible/Book of Virtues is a single template call, so there is nothing on
the page to keep even if we wanted it. A counter that measures lines cannot tell those from a
parser losing ground, which is the thing B54 exists to say.

**And the second-subject family stopped being one question.** Three things nobody had crossed:

- **`entity:Ultra Greedier` is already hand-corrected** in `graph/rules/corrections.json` to
  `mark { column: greed, level: second }` — the exact bit B58 confirmed this morning. We are
  re-stating in a rules file what the dropped section of boss/Ultra Greed says.
- **Four of them have a player id**, named in `catalog/src/heads.rs`: Lazarus Risen 11, Black
  Judas 12, The Soul 17, Dark Esau 39. Not "a second subject the page mentions" — subjects the
  game enumerates.
- **Broken Shovel inverts the entry's premise.** B54 says "a page has one entry"; the Cargo table
  has `Broken Shovel 1` and `Broken Shovel 2`, and `dataset/wiki.json` already holds **two**
  entries with that title, built from the one page. The two dropped sections are exactly the two
  halves that tell the ids apart, so the two entries say the same thing today where they are meant
  to differ.

So the ten split six/three-plus-one rather than standing together, and the shape question for the
six is narrower than the entry had it: whether an `Entry` may be built from a **section**. Left to
the owner, because it decides `Entry`.

- [x] 22 read on their pages, recorded as a test with each page named, and the test shown able to
      fail.
- [x] B54 carries the four-way split and the three findings.
- [ ] The second-subject decision. It is a shape question about `Entry` and nobody but the owner
      should answer it.
- [ ] `Special Locusts` is 173 lines on collectible/Abyss and is named nowhere in `graph` or
      `catalog` — the largest single thing in the family with no id behind it.

### 2026-09-17 (after that) — B22's two counts, and the character who stops being complete

Items 1–3 of B22. Item 4 is the layout and is the only part that needs a window, so it stays
open and says so.

**The entry named the wrong struct and it did not matter, which is worth a sentence.** Item 3
asked for `CharacterRow.started` to become `normal` and `hard`. `CharacterRow` has no count:
the wire carries cells, and the frontend tallies them itself. The field is on `MarksTotals`.
The rename landed there, and the error was harmless only because the two happened to want the
same change — an entry that names a struct it has not opened is a coin flip.

**What the change actually costs, stated as a number:** the reference profile goes from **3
complete characters to 2**. `complete` was "every readable cell has a level" and is now "every
readable cell is hard", which is what the game's own widget means by a full row. Isaac is the
one who leaves: his last cell is a bare 1, so he is 12/12 on the first count and 11/12 on the
second. That single row is the whole entry in miniature, and it is why one number could not
keep doing the job of two.

**Every expected value here was counted on the fixture's digit strings with `awk`**, not read
back out of the module. The method validated itself first: the four numbers the design brief
had already printed — 408 cells, 368 readable, 40 unreadable, 166 with a level — came out of
the same count unchanged, and only then were the new ones (152 hard, 2 complete, and the
per-row and per-column pairs) taken from it. Greed is the column where the split earns its
keep: **16 with a level, 5 hard**, because bit 1 there is Ultra Greedier and not a harder run
of the same mode.

**What needed no change, and that is the interesting half.** The legend already listed five
rows with no "both" and already drew `bits: 3` as hard; `markVisual` already picked the hard
sprite whenever bit 1 was set. The drawing had obeyed the rule all along. Only the *reading*
disagreed with it — `CellStatus.Both`, a tooltip saying "normale e hard", and a bare 2 reading
as "hard, normal missing". A screen can be right in pixels and wrong in words, and the words
are the part with the tests.

- [x] `CellStatus.Both` gone; `completion.cell.both` deleted in both languages.
- [x] `Tally` is `{ normal, hard, readable, complete }`, with `hard ≤ normal ≤ readable` as a
      property over all 34 rows and 12 columns — and a vacuity guard, because that inequality
      is free on a profile that never took a hard mark.
- [x] `MarksTotals.normal` / `.hard`, `types.ts` regenerated, and `cross_check.rs` now asks the
      Python reference for both counts instead of one.
- [x] The KPI strip splits: `marks at normal` and `marks at hard` over one denominator. `both`
      went with `CellStatus.Both` — it counted the overlap of a set with its own superset.
- [ ] The grid still shows the pair in one slot (`normal/readable · hard`). Two real columns
      are item 4 and need `Schermate.dc.html` and a window.
- [ ] Still nobody has measured what bit 1 means outside Greed. `MarkLevel` stays `Base` /
      `Second`, and nothing added today is named after a mode.

### 2026-09-17 — B58, where the alarm was wrong and the thing it stood next to was not

The entry said the mark tables were derived on the 2026 series and might address a neighbour's
cell in the 641 era, and it named two cells as evidence. Both turned out to be ordinary, and
finding out took reading an era the entry had not considered.

**What the entry missed.** `20240606`, a 638-era save of a different profile, holds both of the
shapes it called anomalous. One of them is not even a static oddity but a *transition*:
`[457] 0 → 1`, `Isaac × The Beast`, on 2024-03-05, with `[492] 0 → 2` counting it together with
Eden's on the same day. A shape that occurs in two eras is not an argument for a misalignment in
one of them — though it is not an argument against one either, and that is the trap. The same
observation fits "the tables are right" and "the tables are off by the same k in both", and an
observation that fits both hypotheses is not evidence however much it sounds like it.

**What separated them** was measuring the base in the 638 era directly, the same way it was
measured in the 642: a mark appearing on the day that boss's kill counter rises by exactly as
many. Three windows, two of them exact. 423 and 457, again. Section 2 is 496 cells in the 638
era, 521 in the 641, 523 in the 642 — it only ever grew — so the era between two measured ones is
bracketed. It is an inference and not a third measurement; this machine has one 641-era file and
therefore no window in it at all, which is worth saying because the entry asked for exactly that
file and having it was not the same as being able to walk it.

**The second row of the entry's table dissolved differently, and left something behind.** Four
cells go `1 → 2` across the 638 series. Bit 0 goes *out* as bit 1 comes in, so a value replaces
the one before it rather than accumulating, and `Isaac × Greed = 2` is a Greedier clear that
overwrote the Greed one — which is what "Ultra Greedier implies Greed" looks like in a file that
overwrites. `docs/save-format.md` had the opposite reading written into it, not as a claim but as
the *reason* for a property ("an online clear is also a clear, so bit 2 never stands without bit
0"). The property holds; the reason was wrong, and a wrong reason attached to a true statement is
the kind of thing that survives a long time. The test it named is renamed with it.

**What was actually broken, and it was not in the tables.** The three properties in
`crates/ipc/tests/marks_real.rs` that exist to keep the tables answerable to the series walked
the `rep+` series only, by an explicit decision in the file — *"deliberately one series and not
both"*, which is right about not spanning the edition jump and does not follow. On a machine
whose `rep+` series is one file they return early. That is this machine, and every machine
without 2026 snapshots. So the guard on the three derived bases had been off, everywhere except
the one machine that collected the newest era, and the suite was green throughout. They now walk
each series on its own.

**And the hole that is still there, which is the part worth keeping.** Moving Mother's base to
**422** leaves every real-data property green here. The counters compare counts: the neighbour
lights up on the same day and the arithmetic works. Only index 188 compares identities and the
638 series offers one qualifying window, an Azazel one that never touches Mother. The file's own
comment had predicted this exactly — *"a base off by one ... a neighbouring cell lights up on the
same day and the arithmetic still works"* — which is a good argument for reading the comments
before trusting the greens. What catches 422 is arithmetic on the tables themselves: Delirium's
19-block runs 404..=422, so the two would claim one index. Two pure tests in
`crates/core-save/tests/marks_layout.rs` now say so, and they need **no sample**. That is the
point of them. `samples/` is per-machine, the owner's note that the saves here are old and
unexamined is the ordinary state of this project, and a table's arithmetic does not care.

- [x] `marks_real.rs` walks both series; `the_character_that_won_…`'s floor of 2 becomes a
      declared skip, the way the online bit's absence already was — under the new coverage it
      fires on an ordinary machine, and a statement about `samples/` must not read as one about
      the tables.
- [x] `no_two_cells_of_the_matrix_share_an_index` and
      `the_three_derived_blocks_tile_against_their_neighbours`, both pure.
- [x] `crates/core-save/examples/era_series.rs`, the instrument that re-derives a base in any era.
- [ ] Nobody has measured what bits 0 and 1 mean outside Greed. B58 kept brushing against it and
      it is B22's, not this entry's; nothing here names them.
