...
- **The tree has other sessions' work in it.** Stage by explicit path, never `git add -A`, and say so if the tree holds changes that are not yours. Concretely: while this plan was being written, **N2 ("typed IPC reasons") was merged into `develop`** (`8955dfa`). It did not touch `crates/wiki` — the heart of this plan is unaffected — but it *did* change `ui/src/lib/ipc/types.ts`, which Tasks 2, 3 and 4 edit, and it rewrote 155 lines of `docs/STATUS.md`, which Task 9 edits. Re-read both files before editing them rather than trusting the excerpts quoted here.
