...
Expected: PASS, 25 tests across the two files.
...
- Produces: `ipc::RollView`, `ipc::DrawnView`, `ipc::DrawnTargetView`, `ipc::StatusView`,
  ...
  `ipc::preset_view(&roll::Preset) -> PresetView`.

**`preset_view` is a named public helper, not code inlined in `roll_view`** — Task 7 adds its
inbound twin `preset_from_view` and a round-trip test that calls both by name, and the two
halves of one mapping belong beside each other.
