...
3. **The panel does not reuse `FilterBar.vue`** (Task 9). Its contract is `rows` +
   ...
   `FacetOption[]` — so the option list is still written once. **This is the deviation most
   worth disagreeing with**: §6 asserts the reuse in one clause, and nothing else depends on it.
